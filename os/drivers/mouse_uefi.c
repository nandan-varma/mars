...
static EFI_SIMPLE_POINTER_PROTOCOL *g_simple_protocol;
static EFI_ABSOLUTE_POINTER_PROTOCOL *g_absolute_protocol;
...
void mouse_driver_init(
    ...
    (void)boot_services;
    g_simple_protocol = simple_protocol;
    g_absolute_protocol = absolute_protocol;
    g_ps2_enabled = FALSE;
    ...
    if (g_simple_protocol != NULL) {
        (void)g_simple_protocol->Reset(g_simple_protocol, FALSE);
    }
    if (g_absolute_protocol != NULL) {
        (void)g_absolute_protocol->Reset(g_absolute_protocol, FALSE);
    }

    if (g_simple_protocol == NULL && g_absolute_protocol == NULL) {
        g_ps2_enabled = ps2_mouse_init();
    }
}

static UINTN poll_simple_pointer(input_event_t *events_out, UINTN max_events) {
    if (g_simple_protocol == NULL || events_out == NULL || max_events == 0) {
        return 0;
    }

    EFI_SIMPLE_POINTER_STATE state;
    EFI_STATUS status = g_simple_protocol->GetState(g_simple_protocol, &state);
...
static UINTN poll_absolute_pointer(input_event_t *events_out, UINTN max_events) {
    if (g_absolute_protocol == NULL || events_out == NULL || max_events == 0 || g_absolute_protocol->Mode == NULL) {
        return 0;
    }

    EFI_ABSOLUTE_POINTER_STATE state;
    EFI_STATUS status = g_absolute_protocol->GetState(g_absolute_protocol, &state);
    if (status == EFI_NOT_READY || EFI_ERROR(status)) {
        return 0;
    }

    UINT64 min_x = g_absolute_protocol->Mode->AbsoluteMinX;
    UINT64 max_x = g_absolute_protocol->Mode->AbsoluteMaxX;
    UINT64 min_y = g_absolute_protocol->Mode->AbsoluteMinY;
    UINT64 max_y = g_absolute_protocol->Mode->AbsoluteMaxY;
    ...
    BOOLEAN left_now = (state.ActiveButtons & EFI_ABSP_TouchActive) ? TRUE : FALSE;
    if (left_now != g_left_down && event_count < max_events) {
        events_out[event_count].type = left_now ? INPUT_EVENT_MOUSE_BUTTON_DOWN : INPUT_EVENT_MOUSE_BUTTON_UP;
        events_out[event_count].data.mouse_button.left = left_now;
...
UINTN mouse_driver_poll(input_event_t *events_out, UINTN max_events) {
    ...
    UINTN count = poll_simple_pointer(events_out, max_events);
    if (count > 0) {
        return count;
    }
    ...
    return poll_ps2_mouse(events_out, max_events);
}
...
BOOLEAN mouse_driver_has_simple(void) {
    return g_simple_protocol != NULL;
}

BOOLEAN mouse_driver_has_absolute(void) {
    return g_absolute_protocol != NULL;
}
